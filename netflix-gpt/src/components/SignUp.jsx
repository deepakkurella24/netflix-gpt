    ...
    <>
       ...
       <div className='absolute px-9 py-4 bg-black/70 w-3/12 my-50 left-0 right-0 mx-auto rounded-md '>
            ...
            <form onSubmit={(e)=>e.preventDefault()}>
                {
                  !isSignUp&&<input type="text" ref={userName} placeholder='username'className=' text-white border border-white border-1 bg-gray-1000  placeholder-white w-full py-2 px-1 my-2 rounded-md' />
                }
            ...
            <div>
              ...
              <span className='text-white text-l cursor-pointer hover:underline ' onClick={()=>setIsSignUp(!isSignUp)} >{!isSignUp?"Sign In now.":'Sign Up now.'}</span>
            </div>
       </div>
    </>
  )
